...
void inserir(struct tArvore *arvore, struct tNo *novo) {
    ...
    while(filho != NULL) {
        pai = filho;
        if(strcmp(filho->chave, novo->chave) <= 0) { //se for repetido inclui na subarvore da direita
            filho = filho->direita;
        } else {
...
struct tNo * remover(struct tArvore *arvore, char *chave) {
    struct tNo *pai = NULL, *filho = raiz(arvore), *subs = NULL;

    while(filho != NULL && (strcmp(filho->chave, chave) < 0 || strcmp(filho->chave, chave) > 0)) { //para na que encontrou primeiro
        pai = filho;
